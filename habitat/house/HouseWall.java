...
public final class HouseWall {
	 ...
	 */
	public HouseWall(final Location start, final Location end, Optional<Location> door) {
		// TODO implement exercise 2 (b) here.
		if (start == null || end == null) {
			throw new IllegalArgumentException();
		}
		if (!areInLine(start, end) || !isStartSmallerThanEnd(start, end)) {
			throw new IllegalArgumentException();
		}
		if (start.equals(end)) {
			throw new IllegalArgumentException();
		}
		//door.ifPresent(doorLocation -> {
		//	if (!isValidDoorOnHorizontalWall(doorLocation) && !isValidDoorOnVerticalWall(doorLocation)) {
		//		throw new IllegalArgumentException();
		//	}
		//});
	...
	public void addDoor(final Location newDoor) {
		...
		if (!isValidDoorOnHorizontalWall(newDoor) && !isValidDoorOnVerticalWall(newDoor)) {
			throw new IllegalArgumentException();
		}

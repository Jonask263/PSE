...
public final class HouseBuilder {
	...
	private void buildWall(final HouseWall wall) {
		...
		/*@
		@ loop_invariant saved starting, ending and optional door location for n HouseWalls
		@ if n is the number of already executed loop iterations
		@ decreasing walls.size() - n, if n is the number of already executed loop iterations
		@*/
		for (HouseWall currentWall : walls) {
			Location start = currentWall.getStart();
			Location end = currentWall.getEnd();
			Optional<Location> doorOptional = currentWall.getDoor();
			/*@
			@ loop_invariant executed the next loop n times if n is the number of already executed loop iterations
			@ decreasing HouseWall.getAmountRows() - n, if n is the number of already executed loop iterations
			@*/
			for (int row = start.getRow(); row <= end.getRow(); row++) {
				/*@
				@ loop_invariant placed n wall + doors if n is the number of already executed loop iterations
				@ decreasing scope of the house - n, if n is the number of already executed loop iterations
				@*/
				for (int col = start.getColumn(); col <= end.getColumn(); col++) {
					if(doorOptional.isPresent() && row == doorOptional.get().getRow() && col == doorOptional.get().getColumn()) {
						;
					}else {
					territoryBuilder.wallAt(new Location(row, col));
					}
				}
			}
		}
